<script>
  ...
  async function onclick(selectedImage) {
    ...
    var lc_feature_maps = await n.load(
      "http://localhost:5173/" + selectedImage.lc_feat_path
    );
    var lb_feature_maps = await n.load(
      "http://localhost:5173/" + selectedImage.lb_feat_path
    );
    var la_feature_maps = await n.load(
      "http://localhost:5173/" + selectedImage.la_feat_path
    );
    ...
    lc_feat_array = d3.map(
      lc_feat_array,
      (d) => (d - lc_min) / (lc_max - lc_min)
    );
    lb_feat_array = d3.map(
      lb_feat_array,
      (d) => (d - lb_min) / (lb_max - lb_min)
    );
    la_feat_array = d3.map(
      la_feat_array,
      (d) => (d - la_min) / (la_max - la_min)
    );
    ...
    for (let i = 0; i < 8; i++) {
      ...
      for (let j = 0; j < lc_size; j++) {
        lc_colorhue[idx][i][0] +=
          lc_feat_array[j + i * (lc_feat_array.length / 8)];
      }
      for (let j = 0; j < lb_size; j++) {
        lb_colorhue[idx][i][0] +=
          lb_feat_array[j + i * (lb_feat_array.length / 8)];
      }
      for (let j = 0; j < la_size; j++) {
        la_colorhue[idx][i][0] +=
          la_feat_array[j + i * (la_feat_array.length / 8)];
      }
  ...
  let img_idx = [0, 1, 10, 11, 20, 21, 30, 31];
...
<div class="container">
  ...
  <div class="main">
    ...
    />
    <Details selectedImage={selectedImage_list[7]} />
  </div>
</div>
